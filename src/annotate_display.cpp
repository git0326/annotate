#include <annotate_standalone/annotate_display.h>
...
{
  // Discover the pause_playback service (provided by BagPlayer or rosbag play)
  if (!playback_client_.exists() || !playback_client_.isValid())
  {
    ...
    QString const status_name = "playback service";
    if (services.empty())
    {
      setStatus(rviz::StatusProperty::Warn, status_name,
                "Found no pause_playback ROS service to toggle playback. Open a bag file in the Bag Player panel.");
    }
...
void AnnotateDisplay::onInitialize()
{
  ...
  auto* play_pause =
      new ShortcutProperty("toggle pause", "space", "Toggle play and pause state", shortcuts_property_);
